...
筆 (`paper/brush.ts`): 筆圧で半径。動いていれば進行方向に伸びて後ろへずれ (穂の引きずり)、止まっていれば傾きで楕円。
墨残量はストローク開始時 1 で、筆圧で重み付けした累積長ととどまった時間で減る。
接地判定は紙の高さで、しきい値は墨残量が減るほど・筆圧が低いほど・速いほど上がる (`kasureMin` → `kasureMax`、`kasureSpeed`) ので、
墨のある筆はべったり付き、掠れの筋は紙の凹凸と一致する。墨残量が減る・速いと接地面が進行方向の細い筋に分かれる (毛の割れ)。
