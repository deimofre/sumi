...
export const PAPER_SCHEMA = [
  ...
  // ---- 表面の膜 (筆が置いた墨。細かい格子) ----
  { key: 'soakRate', group: '表面の膜', label: '染み込む速さ', def: 1.2, min: 0.1, max: 5, step: 0.1,
    hint: '膜の水が 1 秒に紙へ染み込む量 (紙が乾いているとき)。紙が飽和している所では待つので、厚い墨ほど長く艶が残る' },
  { key: 'filmEvap', group: '表面の膜', label: '膜の蒸発', def: 0.04, min: 0, max: 0.5, step: 0.01,
  ...
  { key: 'kasureMin', group: '筆', label: '掠れしきい値 (墨あり)', def: 0.0, min: 0, max: 1, step: 0.05,
    hint: '墨残量 1 での接地しきい値。紙の高さがこれ未満の所には墨が付かない。0 なら墨のある筆はべったり付く' },
  { key: 'kasureMax', group: '筆', label: '掠れしきい値 (墨切れ)', def: 0.75, min: 0, max: 1, step: 0.05,
  ...
  { key: 'kasureSpeed', group: '筆', label: '速い筆の掠れ (飛白)', def: 0.35, min: 0, max: 0.8, step: 0.05,
    hint: '速く払うほどしきい値が上がり、毛も割れる。水面モードの「速いほどかすれる」と同じ感触' },
  ...
  { key: 'brushFixFraction', group: '筆', label: 'その場で定着する割合', def: 0.6, min: 0, max: 1, step: 0.05,
    hint: '染み込むとき顔料のうち繊維にその場で定着する割合。残りは水に乗ってにじむ。大きいほど線が濃くにじみが薄い' },
  { key: 'brushWater', group: '筆', label: 'なぞりの水', def: 0.35, min: 0, max: 2, step: 0.05,
    hint: '1 サンプル (短辺の 0.5%) あたり膜に置く水 (墨残量 1 のとき)' },
  { key: 'brushPigment', group: '筆', label: 'なぞりの顔料', def: 0.3, min: 0, max: 1.5, step: 0.01,
    hint: '1 サンプルあたり膜に置く顔料。水との比が墨の濃さ (濃墨・淡墨)' },
  { key: 'holdWater', group: '筆', label: 'とどまりの水 / 秒', def: 3.0, min: 0, max: 15, step: 0.5,
    hint: '押したまま止まっている間の供給。染み込みきらない分が液だまりになり、放した後もにじみを進める' },
  { key: 'holdPigment', group: '筆', label: 'とどまりの顔料 / 秒', def: 2.5, min: 0, max: 8, step: 0.1,
    hint: '' },
  // ---- 表示 ----
