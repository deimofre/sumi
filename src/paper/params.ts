...
export const PAPER_SCHEMA = [
  ...
  { key: 'filmOpacity', group: '表面の膜', label: '膜の墨の濃さ', def: 5.0, min: 0.5, max: 12, step: 0.5,
    hint: '膜の中の顔料 → 不透明度の係数' },
  // ---- 筆 ----
  { key: 'brushRadius', group: '筆', label: '基準半径', def: 0.04, min: 0.004, max: 0.08, step: 0.001,
    hint: '筆圧 1 のときの半径 (短辺 = 1)。筆の腹まで使ったときの太さ' },
  { key: 'pressureMin', group: '筆', label: '筆圧 0 の半径', def: 0.08, min: 0.02, max: 1, step: 0.02,
    hint: '穂先だけが触れたときの太さ (基準半径に対する割合)' },
  { key: 'pressureCurve', group: '筆', label: '筆圧カーブ', def: 1.1, min: 0.3, max: 2.5, step: 0.05,
